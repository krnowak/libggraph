    ...
    if (_data_pair_is_valid(data_pair))
    {
      ...
      switch (created)
      {
        case G_SGRAPH_NODE_NONE:
        {
          ...
          for (iter2 = 0; iter2 < (separate_graphs->len - 1); iter2++)
          {
            for (iter3 = iter2 + 1; iter3 < separate_graphs->len; iter3++)
            {
              ...
              first_node = g_ptr_array_index(separate_graphs, iter2);
              second_node = g_ptr_array_index(separate_graphs, iter3);
              if (!_g_sgraph_node_are_separate(first_node, second_node))
              {
                g_ptr_array_remove_index_fast(separate_graphs, iter3);
                hit = TRUE;
                break;
              }
            }
        ...
        case G_SGRAPH_NODE_BOTH:
        {
          ...
          g_ptr_array_add(separate_graphs, first_node);
          break;
        }
      }
    }
                      ...
                      GSGraphNode* other_sgraph_node)
{
  ...
  {
    GSGraphNode* node = g_ptr_array_index(s_n, iter);
    if (node == other_sgraph_node)
    {
      return;
    }
  }
  g_ptr_array_add(s_n, other_sgraph_node);
 ...
 * ones, so this function returns #GPtrArray holding pointers to all
...
g_sgraph_node_remove(GSGraphNode* sgraph_node)
{
  GPtrArray* s_n;
  GPtrArray* c_n;
  GPtrArray* separate_sgraphs;
  guint iter;
  
  g_return_val_if_fail(sgraph_node != NULL, NULL);
  
  separate_sgraphs = g_ptr_array_new();
  s_n = sgraph_node->neighbours;
  c_n = g_ptr_array_sized_new(s_n->len);
  for (iter = 0; iter < s_n->len; iter++)
  {
    g_ptr_array_add(c_n, g_ptr_array_index(s_n, iter));
  }
  for (iter = 0; iter < c_n->len; iter++)
  {
    GSGraphNode* other_sgraph_node = g_ptr_array_index(c_n, iter);
    if (g_sgraph_node_break_connection(sgraph_node, other_sgraph_node))
    {
      g_ptr_array_add(separate_sgraphs, other_sgraph_node);
    }
  }
  g_sgraph_node_free(sgraph_node);
  g_ptr_array_free(c_n, TRUE);
...
g_sgraph_node_copy_deep(GSGraphNode* sgraph_node,
                        ...
                        gpointer user_data)
{
  GPtrArray* sgraph_node_array = _g_sgraph_node_array(sgraph_node);
  ...
  {
    GSGraphNode* node = g_ptr_array_index(sgraph_node_array, iter);
  ...
  for (iter = 0; iter < sgraph_node_array->len; iter++)
  {
    ...
    GPtrArray* dup_neighbours = g_ptr_array_sized_new(node->neighbours->len);
    guint iter2;
    for (iter2 = 0; iter2 < node->neighbours->len; iter++)
    {
      GSGraphNode* neighbour = g_ptr_array_index(node->neighbours, iter2);
      GSGraphNode* dup_neighbour = g_hash_table_lookup(nodes_to_dups, neighbour);
      g_ptr_array_add(dup_neighbours, dup_neighbour);
    }
  ...
  g_ptr_array_free(sgraph_node_array, TRUE);
  return dup_graph;
}
                               ...
                               GSGraphNode* other_sgraph_node)
{
  ...
  for (iter = 0; iter < s_n->len; iter++)
  {
    ...
    if (node == other_sgraph_node)
    {
      g_ptr_array_remove_index(s_n, iter);
      found = TRUE;
      break;
    }
  }
  ...
  {
    GSGraphNode* node = g_ptr_array_index(o_n, iter);
    if (node == sgraph_node)
    {
      g_ptr_array_remove_index(o_n, iter);
      break;
    }
  }
 ...
 *
 * Returns: a #GPtrArray* that must be freed with g_ptr_array_free().
 */
GPtrArray*
                          ...
                          GEqualFunc func)
{
  ...
  matching_sgraphs = g_ptr_array_new();
  sgraph_node_array = _g_sgraph_node_array(sgraph_node);
  for (iter = 0; iter < sgraph_node_array->len; iter++)
  {
    GSGraphNode* node = g_ptr_array_index(sgraph_node_array, iter);
    ...
    {
      g_ptr_array_add(matching_sgraphs, node);
    }
  }
  g_ptr_array_free(sgraph_node_array, TRUE);
  if (!matching_sgraphs->len)
  {
    g_ptr_array_free(matching_sgraphs, TRUE);
 ...
 * as second. If an element is found it is added to a #GPtrArray.
 *
 * Returns: a #GPtrArray* that must be freed with g_ptr_array_free().
 */
GPtrArray*
                               ...
                               GSGraphNodeEqualFunc func)
{
  ...
  for (iter = 0; iter < sgraph_node_array->len; iter++)
  {
    GSGraphNode* other_node = g_ptr_array_index(sgraph_node_array, iter);
    if ((*func)(other_node, node))
    {
      g_ptr_array_add(matching_sgraphs, other_node);
    }
  }
  g_ptr_array_free(sgraph_node_array, TRUE);
  if (!matching_sgraphs->len)
...
{
  GHashTable* visited_nodes = g_hash_table_new(NULL, NULL);
  GPtrArray* sgraph_node_array = g_ptr_array_new();
                                      ...
                                      GHashTable* visited_nodes)
{
  ...
  g_hash_table_insert(visited_nodes, sgraph_node, NULL);
  g_ptr_array_add(sgraph_node_array, sgraph_node);
  s_n = sgraph_node->neighbours;
  for (iter = 0; iter < s_n->len; iter++)
  {
    GSGraphNode* node = g_ptr_array_index(s_n, iter);
